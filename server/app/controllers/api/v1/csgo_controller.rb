...
module Api::V1
  class CsgoController < ApiController
    ...
    def self.create_lobby_link(player_object)
      format('steam://joinlobby/%s/%s/%s', player_object['gameid'], player_object['lobbysteamid'], player_object['steamid'])
    end

    def join_game
      if params['id']
        response = Unirest.get format('http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=%s&steamids=%s', ENV['STEAM_KEY'], params['id'])
    ...
    def self.pollLobby
      ...
      for lobby in result
        response = Unirest.get format('http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=%s&steamids=%s', ENV['STEAM_KEY'], lobby.id)
        ...
        next unless lobbyid && (lobby.lobbyid != lobbyid)
